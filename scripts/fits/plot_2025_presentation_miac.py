...
def _legend_handle(color: str, label: str):
    return Line2D([0], [0], color=color, linewidth=2.1, marker='o', markerfacecolor='white', markeredgecolor=color, markersize=5.5, label=label)
...
def make_plot(group: dict[str, object], combo_map: dict[tuple[str, str], dict[str, object]]) -> Path:
    ...
    for spec in group['series']:
        ...
        max_y = max(max_y, float(np.max(values_exp)) if len(values_exp) else 0.0, float(np.max(curve)) if len(curve) else 0.0)
    ...
    ax.text(0.01, 0.99, 'Open circles: data\nSolid lines: 2025 fit', transform=ax.transAxes, ha='left', va='top', fontsize=8.5, color='black')
    if notes:
        ax.text(0.99, 0.01, '; '.join(notes), transform=ax.transAxes, ha='right', va='bottom', fontsize=8, color='black')

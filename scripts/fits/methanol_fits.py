...
def _load_nabr_methanol_data():
    ...
    return molal[order], gamma_exp[order], data_path
...
def test_methanol_miac_m_fit():
    ...
    molal_exp, gamma_exp, data_path = _load_nabr_methanol_data()

    out_dir = data_path.parent / "plot_fits"
    out_dir.mkdir(parents=True, exist_ok=True)
    plot_path = out_dir / "maic_m_methanol_NaBr_fit.png"
    ...
    for rule, aard, rmse in metrics_rows:
        ax.plot(
            ...
            linewidth=1.8,
            linestyle="--",
            label=f"rule {rule}: AARD={aard:.2f}% RMSE={rmse:.4f}",
        )
...
def test_methanol_dh_and_born_model_variants():
    ...
    molal_exp, _, _ = _load_nabr_methanol_data()

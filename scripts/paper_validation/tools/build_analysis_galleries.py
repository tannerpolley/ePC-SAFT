...
<body>
  ...
  <script>
    (() => {{
      ...
      document.getElementById("expand-all").addEventListener("click", () => {{ for (const folder of makeFolderMap().keys()) expanded.add(folder); renderTree(); }});
      document.getElementById("collapse-all").addEventListener("click", () => {{ expanded.clear(); expanded.add(""); renderTree(); }});
      document.getElementById("clear-selection").addEventListener("click", () => {{ selected.clear(); renderTree(); renderGallery(); }});
      ...
      tileSizeEl.addEventListener("input", renderGallery);

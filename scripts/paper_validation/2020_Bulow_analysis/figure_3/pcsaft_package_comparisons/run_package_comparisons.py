...
SOURCE_ORDER = ("paper", "pcsaft", "feos", "clapeyron")
SOURCE_LABELS = {
    "paper": "Paper",
...
def _load_pcsaft_data(ions: list[str]) -> dict[str, np.ndarray]:
    ...
    with SUMMARY_PATH.open("w", newline="", encoding="utf-8") as handle:
        ...
        for ion_idx, ion in enumerate(ions):
            for metric in METRICS:
                ...
                pcsaft_value = float(source_values["pcsaft"][metric][ion_idx])
                for source in SOURCE_ORDER:
                    ...
                    writer.writerow(
                        {
                            ...
                            "delta_vs_pcsaft_kj_mol": f"{(value - pcsaft_value):.12f}",
                            "pct_vs_pcsaft": f"{_pct_delta(value, pcsaft_value):.12f}",
...
def _plot_term(
    ...
    ax.set_title(f"Bulow 2020 Figure 3 {term_label}: paper vs PC-SAFT vs feos vs Clapeyron ($\\mu$ basis)")
...
def _plot_comprehensive(ions: list[str], source_values: dict[str, dict[str, np.ndarray]]) -> None:
    ...
    ax.set_ylabel(r"Contribution to $\Delta G_{\mathrm{hyd},i}^{\infty}$ / kJ mol$^{-1}$ ($\mu$ basis)")
    ax.set_title("Bulow 2020 Figure 3 Comprehensive: paper vs PC-SAFT vs feos vs Clapeyron")
...
def main() -> None:
    ...
    pcsaft_values = _load_pcsaft_data(ions)

...
REDUCED_SOURCES = ('paper', 'epcsaft', 'feos')
REDUCED_LABELS = {
    'paper': 'Paper',
...
def _plot_term_reduced(
    ...
    ax.set_title(f'Bulow 2020 Figure 3 {term_label}: paper vs ePC-SAFT vs feos ($\\mu$ basis)')
...
def _plot_comprehensive_reduced(ions: list[str], source_values: dict[str, dict[str, np.ndarray]]) -> Path:
    ...
    ax.set_ylabel(r'Contribution to $\Delta G_{\mathrm{hyd},i}^{\infty}$ / kJ mol$^{-1}$ ($\mu$ basis)')
    ax.set_title('Bulow 2020 Figure 3 Comprehensive: paper vs ePC-SAFT vs feos')
...
def main() -> None:
    ...
    epcsaft_values = rpc._load_epcsaft_data(ions)

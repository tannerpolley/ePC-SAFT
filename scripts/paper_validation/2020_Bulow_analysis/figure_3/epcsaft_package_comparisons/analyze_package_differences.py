...
def _current_repo_term_row(frame: common.Table, totals: common.Table, ion: str, contribution: str) -> dict[str, float | str]:
    ...
    if contribution == "total":
        ...
        return {
            ...
            "epcsaft_mu_contr": mu_sum,
            "epcsaft_mu_manual_sum": mu_sum,
            "epcsaft_a_contr": "",
            "epcsaft_z_contr": "",
            "epcsaft_composition_derivative_residual_helmholtz_contr": "",
            "epcsaft_sum_xj_dadx_contr": "",
            "epcsaft_z_correction": lnfug_sum - mu_sum,
            "epcsaft_lnfug_contr": lnfug_sum,
            "epcsaft_total": overlay.gsolv_ion("advanced", ion, "water"),
        }
    ...
    epcsaft_mu = RT_KJMOL * mu_term
    return {
        "paper_mu_contr": _paper_value(frame, contribution, ion),
        "epcsaft_mu_contr": epcsaft_mu,
        "epcsaft_mu_manual_sum": RT_KJMOL * (a_term + z_term + dadx_term - sum_x_dadx_term),
        "epcsaft_a_contr": RT_KJMOL * a_term,
        "epcsaft_z_contr": RT_KJMOL * z_term,
        "epcsaft_composition_derivative_residual_helmholtz_contr": RT_KJMOL * dadx_term,
        "epcsaft_sum_xj_dadx_contr": RT_KJMOL * sum_x_dadx_term,
        "epcsaft_z_correction": RT_KJMOL * (lnfug_term - mu_term),
        "epcsaft_lnfug_contr": RT_KJMOL * lnfug_term,
        "epcsaft_total": overlay.gsolv_ion("advanced", ion, "water"),
    }
...
def _build_breakdown_rows() -> list[dict[str, object]]:
    ...
    for ion in TARGET_IONS:
        for contribution in TERM_ORDER:
            ...
            row["feos_minus_epcsaft_mu"] = float(row["feos_mu_contr"]) - float(row["epcsaft_mu_contr"])
            row["clapeyron_minus_epcsaft_mu"] = float(row["clapeyron_mu_contr"]) - float(row["epcsaft_mu_contr"])
            rows.append(row)
    return rows
...
def _write_breakdown_csv(rows: list[dict[str, object]]) -> Path:
    ...
    fieldnames = [
        ...
        "clapeyron_mu_total",
        "clapeyron_total",
        "feos_minus_epcsaft_mu",
        "clapeyron_minus_epcsaft_mu",
    ]
...
def _clapeyron_param_snapshot() -> dict[str, float]:
    unlike_adv = _load_clapeyron_csv(CLAPEYRON_ROOT / "database" / "SAFT" / "ePCSAFT" / "ePCSAFTAdv" / "ePCSAFTAdv_unlike.csv")
...
def _render_report(rows: list[dict[str, object]]) -> str:
    ...
    for ion in TARGET_IONS:
        ...
        lines.append(
            f"- `{ion}`: `epcsaft total - adjusted_sum = {float(total['epcsaft_total']) - float(total['epcsaft_lnfug_contr']):+.6f}` kJ/mol, "
    ...
    for ion in TARGET_IONS:
        for contr in ("hc", "disp", "assoc", "born"):
            row = find(ion, contr)
            lines.append(
                f"- `{ion}` `{contr}` mu term: `epcsaft = {float(row['epcsaft_mu_contr']):+.6f}`, "
...
def main() -> None:
    if not FEOS_RAW_PATH.exists() or not CLAPEYRON_RAW_PATH.exists():
        raise FileNotFoundError(
            "Expected feos_raw.json and clapeyron_raw.json in epcsaft_pressureackage_comparisons. "

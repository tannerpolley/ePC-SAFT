...
from scripts._epcsaft_oop import epcsaft_fugacity_coefficient_terms
...
def _epcsaft_base():
    ...
    def term_state(x_new: np.ndarray):
        ...
        return epcsaft_fugacity_coefficient_terms(overlay.T_REF, rho, x_new, params_new)
    ...
    common = _common_fields(
        ...
        x_target=float(x[ePCSAFT_TARGET]),
        x_counter=float(x[ePCSAFT_COUNTER]),
        x_water=float(x[ePCSAFT_WATER]),
        density_mol_m3=rho,
        compressibility_factor=float(terms["z_total"]),
        sigma_target=float(params["s"][ePCSAFT_TARGET]),
        sigma_counter=float(params["s"][ePCSAFT_COUNTER]),
        sigma_water=float(params["s"][ePCSAFT_WATER]),
        epsilon_target=float(params["e"][ePCSAFT_TARGET]),
        epsilon_counter=float(params["e"][ePCSAFT_COUNTER]),
        epsilon_water=float(params["e"][ePCSAFT_WATER]),
        born_radius_target=float(params["d_born"][ePCSAFT_TARGET]),
        born_radius_counter=float(params["d_born"][ePCSAFT_COUNTER]),
        water_target_k=float(params["k_ij"][ePCSAFT_WATER][ePCSAFT_TARGET]),
        water_counter_k=float(params["k_ij"][ePCSAFT_WATER][ePCSAFT_COUNTER]),
        target_counter_k=float(params["k_ij"][ePCSAFT_TARGET][ePCSAFT_COUNTER]),
        dielectric_target=float(params.get("dielc", np.zeros(3))[ePCSAFT_TARGET]),
        dielectric_counter=float(params.get("dielc", np.zeros(3))[ePCSAFT_COUNTER]),
        dielectric_water=float(params.get("dielc", np.zeros(3))[ePCSAFT_WATER]),
    )
    return state, geom, terms, x, common, term_state


def _epcsaft_exact_extra(contribution: str, state, geom) -> dict[str, object]:
    if contribution == "hc":
        row = next(row for row in repo_diag._hc_breakdown(state, geom, FRAME) if row["ion"] == ION)
        out = {f"epcsaft_exact_{key}": value for key, value in row.items() if key != "ion"}
        out["epcsaft_geom_m_avg"] = float(geom["m_avg"])
        out["epcsaft_geom_eta"] = float(geom["eta"])
        out["epcsaft_geom_ares_hs_kj_mol"] = repo_diag._kjmol(float(geom["ares_hs"]))
        out["epcsaft_geom_zhs_kj_mol"] = repo_diag._kjmol(float(geom["zhs"]))
        return out
    if contribution == "disp":
        ...
        out["epcsaft_geom_m_avg"] = float(geom["m_avg"])
        out["epcsaft_geom_eta"] = float(geom["eta"])
        out["epcsaft_geom_i1"] = float(geom["i1"])
        out["epcsaft_geom_i2"] = float(geom["i2"])
        out["epcsaft_geom_c1"] = float(geom["c1"])
        out["epcsaft_geom_c2"] = float(geom["c2"])
        out["epcsaft_geom_m2es3"] = float(geom["m2es3"])
        out["epcsaft_geom_m2e2s3"] = float(geom["m2e2s3"])
        return out
    if contribution == "assoc":
        ...
        return {f"epcsaft_exact_{key}": value for key, value in row.items() if key != "ion"}
    return {}


def _epcsaft_row(contribution: str, state, geom, terms, x, common, term_state) -> dict[str, object]:
    ...
    row.update(
        {
            ...
            "target_mu_kj_mol": repo_diag._kjmol(float(mu_arr[ePCSAFT_TARGET])),
            "counter_mu_kj_mol": repo_diag._kjmol(float(mu_arr[ePCSAFT_COUNTER])),
            "water_mu_kj_mol": repo_diag._kjmol(float(mu_arr[ePCSAFT_WATER])),
            "target_lnfug_kj_mol": repo_diag._kjmol(float(lnfug_arr[ePCSAFT_TARGET])),
            "target_z_correction_kj_mol": repo_diag._kjmol(float(lnfug_arr[ePCSAFT_TARGET] - mu_arr[ePCSAFT_TARGET])),
            ...
            "target_mu_minus_a_z_kj_mol": repo_diag._kjmol(float(mu_arr[ePCSAFT_TARGET] - a_term - z_term)),
            "counter_mu_minus_a_z_kj_mol": repo_diag._kjmol(float(mu_arr[ePCSAFT_COUNTER] - a_term - z_term)),
            "water_mu_minus_a_z_kj_mol": repo_diag._kjmol(float(mu_arr[ePCSAFT_WATER] - a_term - z_term)),
            "simplex_fd_target_da_dx_kj_mol": repo_diag._kjmol(
                _forward_simplex_fd(x, ePCSAFT_TARGET, ePCSAFT_WATER, lambda x_new: float(term_state(x_new)[f"a_{suffix}"]))
            ),
            "simplex_fd_counter_da_dx_kj_mol": repo_diag._kjmol(
                _forward_simplex_fd(x, ePCSAFT_COUNTER, ePCSAFT_WATER, lambda x_new: float(term_state(x_new)[f"a_{suffix}"]))
            ),
            "simplex_fd_water_da_dx_kj_mol": repo_diag._kjmol(
                _forward_simplex_fd(x, ePCSAFT_WATER, ePCSAFT_TARGET, lambda x_new: float(term_state(x_new)[f"a_{suffix}"]))
            ),
            "epcsaft_exact_target_dadx_kj_mol": repo_diag._kjmol(float(np.asarray(terms[f"dadx_{suffix}"], dtype=float)[ePCSAFT_TARGET])),
            "epcsaft_exact_sum_xj_dadx_kj_mol": repo_diag._kjmol(float(terms[f"sum_x_dadx_{suffix}"])),
            "epcsaft_exact_resid_a_plus_z_minus_sumxj_dadx_kj_mol": repo_diag._kjmol(float(a_term + z_term - float(terms[f"sum_x_dadx_{suffix}"]))),
        }
    )
    row.update(_epcsaft_exact_extra(contribution, state, geom))
    return row
...
def main() -> None:
    pc_state, pc_geom, pc_terms, pc_x, pc_common, pc_term_state = _epcsaft_base()
    ...
    for contribution in OUTPUTS:
        pc_row = _epcsaft_row(contribution, pc_state, pc_geom, pc_terms, pc_x, pc_common, pc_term_state)

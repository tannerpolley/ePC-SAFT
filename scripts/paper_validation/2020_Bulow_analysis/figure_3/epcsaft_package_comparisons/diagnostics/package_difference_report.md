...
This report compares the Figure 3 contribution bookkeeping for `Na+` and `Cl-` across the current repo `ePC-SAFT`, `feos`, and `Clapeyron.jl` after correcting the package-side total check to use the fugacity-basis sum where it is reconstructible.

## Basis Checks

- `Na+`: `epcsaft total - adjusted_sum = +0.000000` kJ/mol, `feos total - adjusted_sum = -0.000000` kJ/mol, `clapeyron total - adjusted_sum = -0.000000` kJ/mol.
- `Cl-`: `epcsaft total - adjusted_sum = +0.000000` kJ/mol, `feos total - adjusted_sum = -0.000000` kJ/mol, `clapeyron total - adjusted_sum = +0.000000` kJ/mol.

The `epcsaft` and `Clapeyron` corrected sums close their totals to roundoff. `feos` does not: even after applying the same $-\frac{Z^\alpha}{Z-1}\ln Z$ correction using its pressure contributions, the reconstructed sum still misses the package total by about `-40.962` kJ/mol for both `Na+` and `Cl-`. That means the exposed `feos` contribution labels are not equivalent to the current repo's per-term $\mu^\alpha/Z^\alpha$ bookkeeping.

## Term-Level Observations

- `Na+` `hc` mu term: `epcsaft = +24.398548`, `feos = +24.398548`, `clapeyron = +30.719180` kJ/mol.
- `Na+` `disp` mu term: `epcsaft = -28.733397`, `feos = -28.733397`, `clapeyron = -28.306436` kJ/mol.
- `Na+` `assoc` mu term: `epcsaft = -3.215655`, `feos = -3.215655`, `clapeyron = -4.370860` kJ/mol.
- `Na+` `born` mu term: `epcsaft = -552.066713`, `feos = -552.095649`, `clapeyron = -408.223145` kJ/mol.
- `Cl-` `hc` mu term: `epcsaft = +23.287466`, `feos = +23.287466`, `clapeyron = +28.759551` kJ/mol.
- `Cl-` `disp` mu term: `epcsaft = -30.085203`, `feos = -30.085203`, `clapeyron = -29.684460` kJ/mol.
- `Cl-` `assoc` mu term: `epcsaft = -3.018089`, `feos = -3.018089`, `clapeyron = -4.007542` kJ/mol.
- `Cl-` `born` mu term: `epcsaft = -565.527846`, `feos = -565.557488`, `clapeyron = -354.060342` kJ/mol.
...
- The extractor uses `ESElectrolyte + pharmaePCSAFT + DHBorn + LinMixRSP`, which is the closest composable analogue in-tree, but it is not the same implementation as the current repo's native `2020_Bulow` path.
...
- Detailed cross-package CSV: `C:\Users\Tanner\Documents\git\ePC-SAFT\scripts\paper_validation\2020_Bulow_analysis\figure_3\epcsaft_package_comparisons\diagnostics\figure3_package_mu_breakdown_comparison.csv`
- Main comparison totals/plots: `C:\Users\Tanner\Documents\git\ePC-SAFT\scripts\paper_validation\2020_Bulow_analysis\figure_3\epcsaft_package_comparisons`

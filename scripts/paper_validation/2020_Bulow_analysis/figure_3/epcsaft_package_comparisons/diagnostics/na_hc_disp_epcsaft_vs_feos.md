...
- Patched `feos` hard-chain now matches the current repo full $\tilde{\mu}^{hc}$ branch to roundoff: `feos - epcsaft mu = 4.914806340e-09` kJ/mol.
- Patched `feos` dispersion now matches the current repo full $\tilde{\mu}^{disp}$ branch to roundoff: `feos - epcsaft mu = -5.444082518e-09` kJ/mol.
...
- CSV: `C:\Users\Tanner\Documents\git\ePC-SAFT\scripts\paper_validation\2020_Bulow_analysis\figure_3\epcsaft_package_comparisons\diagnostics\na_hc_disp_epcsaft_vs_feos.csv`

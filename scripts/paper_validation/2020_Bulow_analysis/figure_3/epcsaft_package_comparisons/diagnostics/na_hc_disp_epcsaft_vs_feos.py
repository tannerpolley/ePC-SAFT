...
try:
    ...
    raise RuntimeError("feos and si_units must be installed in the ePC-SAFT conda environment.") from exc
...
CSV_PATH = SCRIPT_DIR / "na_hc_disp_epcsaft_vs_feos.csv"
MD_PATH = SCRIPT_DIR / "na_hc_disp_epcsaft_vs_feos.md"
...
def _epcsaft_numerical_terms(state: dict[str, object]) -> dict[str, np.ndarray | float]:
    ...
    return epcsaft_fugacity_coefficient_terms(T_REF, float(state["rho_inf"]), np.asarray(state["x_inf"], dtype=float), params_num)


def _epcsaft_hc_aux(state: dict[str, object], geom: dict[str, object]) -> dict[str, float]:
...
def _epcsaft_disp_aux(state: dict[str, object], geom: dict[str, object]) -> dict[str, float]:
    m = np.asarray(geom["m"], dtype=float)
    d = np.asarray(geom["d"], dtype=float)
    eta = float(geom["eta"])
    m_avg = float(geom["m_avg"])
...
def _collect_rows() -> list[dict[str, object]]:
    ...
    pc_terms_num = _epcsaft_numerical_terms(pc_state)
    ...
    add("state", "epcsaft", "state", "component_order", ",".join(pc_state["species"]))
    add("state", "epcsaft", "state", "rho_inf_mol_m3", float(pc_state["rho_inf"]))
    x_pc = np.asarray(pc_state["x_inf"], dtype=float)
    for i, sp in enumerate(pc_state["species"]):
        add("state", "epcsaft", "state", f"x_inf_{sp}", float(x_pc[i]))

    for contr in ("hc", "disp"):
        add("branch", "epcsaft", contr, "a_kj_mol", _kjmol(float(pc_terms[f"a_{contr}"])))
        add("branch", "epcsaft", contr, "z_red", float(pc_terms[f"z_{contr}"]))
        add("branch", "epcsaft", contr, "z_kj_mol", _kjmol(float(pc_terms[f"z_{contr}"])))
        add("branch", "epcsaft", contr, "dadx_analytical_kj_mol", _kjmol(float(np.asarray(pc_terms[f"dadx_{contr}"], dtype=float)[pc_idx])))
        add("branch", "epcsaft", contr, "dadx_numerical_kj_mol", _kjmol(float(np.asarray(pc_terms_num[f"dadx_{contr}"], dtype=float)[pc_idx])))
        add(
            "branch",
            "epcsaft",
            contr,
        ...
        add("branch", "epcsaft", contr, "sum_xj_dadx_kj_mol", _kjmol(float(pc_terms[f"sum_x_dadx_{contr}"])))
        add("branch", "epcsaft", contr, "mu_kj_mol", _kjmol(float(np.asarray(pc_terms[f"mu_{contr}"], dtype=float)[pc_idx])))
        add(
            "branch",
            "epcsaft",
            contr,
    ...
    for key, value in _epcsaft_hc_aux(pc_state, pc_geom).items():
        add("aux", "epcsaft", "hc", key, value)
    for key, value in _epcsaft_disp_aux(pc_state, pc_geom).items():
        add("aux", "epcsaft", "disp", key, value)
    ...
    for contr in ("hc", "disp"):
        ...
        add(
            ...
            "feos_vs_epcsaft",
            contr,
            "feos_mu_public_minus_epcsaft_mu_kj_mol",
        ...
        add(
            ...
            "feos_vs_epcsaft",
            contr,
            "feos_mu_public_minus_epcsaft_composition_derivative_residual_helmholtz_kj_mol",
...
def _write_md(rows: list[dict[str, object]]) -> None:
    pc_mu_disp = _get(rows, "epcsaft", "disp", "mu_kj_mol")
    pc_dadx_disp = _get(rows, "epcsaft", "disp", "dadx_analytical_kj_mol")
    fe_mu_disp = _get(rows, "feos", "disp", "mu_public_kj_mol")
    fe_fd_disp = _get(rows, "feos", "disp", "dadx_fd_h1e-06_kj_mol")
    pc_mu_hc = _get(rows, "epcsaft", "hc", "mu_kj_mol")
    pc_dadx_hc = _get(rows, "epcsaft", "hc", "dadx_analytical_kj_mol")
    ...
    lines = [
        ...
        f"- Patched `feos` hard-chain now matches the current repo full $\\tilde{{\\mu}}^{{hc}}$ branch to roundoff: `feos - epcsaft mu = {fe_mu_hc - pc_mu_hc:.9e}` kJ/mol.",
        f"- Patched `feos` dispersion now matches the current repo full $\\tilde{{\\mu}}^{{disp}}$ branch to roundoff: `feos - epcsaft mu = {fe_mu_disp - pc_mu_disp:.9e}` kJ/mol.",

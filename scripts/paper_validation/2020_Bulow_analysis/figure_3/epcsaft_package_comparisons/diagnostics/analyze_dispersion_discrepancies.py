...
def _write_dispersion_table(rows: list[dict[str, str]], clap_scan: dict) -> list[dict[str, float]]:
    ...
    with DISPERSION_TABLE_PATH.open("w", newline="", encoding="utf-8") as handle:
        fieldnames = [
            ...
            "feos_minus_epcsaft_kj_mol",
            "clapeyron_mixed_minus_epcsaft_kj_mol",
            "clapeyron_repo_override_minus_epcsaft_kj_mol",
        ]
        ...
        for ion in IONS:
            row = {
                ...
                "epcsaft_disp_kj_mol": _comparison_value(rows, ion, "epcsaft", "disp"),
            ...
            row["feos_minus_epcsaft_kj_mol"] = row["feos_disp_kj_mol"] - row["epcsaft_disp_kj_mol"]
            row["clapeyron_mixed_minus_epcsaft_kj_mol"] = row["clapeyron_mixed_disp_kj_mol"] - row["epcsaft_disp_kj_mol"]
            row["clapeyron_repo_override_minus_epcsaft_kj_mol"] = (
                row["clapeyron_repo_override_disp_kj_mol"] - row["epcsaft_disp_kj_mol"]
            )
...
def _write_feos_audit(feos_payload: dict, breakdown_rows: list[dict[str, str]]) -> list[dict[str, float]]:
    ...
    with FEOS_AUDIT_PATH.open("w", newline="", encoding="utf-8") as handle:
        fieldnames = [
            "ion",
            "feos_hc_kj_mol",
            "epcsaft_mu_hc_kj_mol",
            "epcsaft_composition_derivative_residual_helmholtz_hc_kj_mol",
            "feos_minus_epcsaft_mu_hc_kj_mol",
            "feos_minus_epcsaft_composition_derivative_residual_helmholtz_hc_kj_mol",
            "feos_disp_kj_mol",
            "epcsaft_mu_disp_kj_mol",
            "epcsaft_composition_derivative_residual_helmholtz_disp_kj_mol",
            "feos_minus_epcsaft_mu_disp_kj_mol",
            "feos_minus_epcsaft_composition_derivative_residual_helmholtz_disp_kj_mol",
        ]
        writer = csv.DictWriter(handle, fieldnames=fieldnames)
        writer.writeheader()
        for ion in IONS:
            ...
            row = {
                ...
                "epcsaft_mu_hc_kj_mol": float(hc_row["epcsaft_mu_contr"]),
                "epcsaft_composition_derivative_residual_helmholtz_hc_kj_mol": float(hc_row["dadx_contr"]),
                "feos_disp_kj_mol": float(feos_row["disp"]),
                "epcsaft_mu_disp_kj_mol": float(disp_row["epcsaft_mu_contr"]),
                "epcsaft_composition_derivative_residual_helmholtz_disp_kj_mol": float(disp_row["dadx_contr"]),
            }
            row["feos_minus_epcsaft_mu_hc_kj_mol"] = row["feos_hc_kj_mol"] - row["epcsaft_mu_hc_kj_mol"]
            row["feos_minus_epcsaft_composition_derivative_residual_helmholtz_hc_kj_mol"] = row["feos_hc_kj_mol"] - row["epcsaft_composition_derivative_residual_helmholtz_hc_kj_mol"]
            row["feos_minus_epcsaft_mu_disp_kj_mol"] = row["feos_disp_kj_mol"] - row["epcsaft_mu_disp_kj_mol"]
            row["feos_minus_epcsaft_composition_derivative_residual_helmholtz_disp_kj_mol"] = row["feos_disp_kj_mol"] - row["epcsaft_composition_derivative_residual_helmholtz_disp_kj_mol"]
            writer.writerow({key: f"{value:.12f}" if key != "ion" else value for key, value in row.items()})
            out_rows.append(row)
    return out_rows
...
def _write_clapeyron_fluoride_audit(clap_scan: dict) -> list[dict[str, float]]:
    ...
    with CLAPEYRON_FLUORIDE_PATH.open("w", newline="", encoding="utf-8") as handle:
        fieldnames = [
            ...
            "water_target_k",
            "counter_target_k",
            "delta_vs_epcsaft_repo_disp_kj_mol",
        ...
        writer = csv.DictWriter(handle, fieldnames=fieldnames)
        writer.writeheader()
        epcsaft_disp = overlay.contribution_breakdown("advanced", "F-", "water", basis="mu")["disp"]
        for config in ("mixed_current", "advanced_only", "repo_unlike_override"):
            ...
            row["delta_vs_epcsaft_repo_disp_kj_mol"] = row["disp_kj_mol"] - epcsaft_disp
...
def _plot_all_ions(disp_rows: list[dict[str, float]]) -> None:
    ...
    order = [
        ...
        ("epcsaft", "epcsaft_disp_kj_mol", "#111111"),
...
def _plot_fluoride(rows: list[dict[str, float]], raw_rows: list[dict[str, str]]) -> None:
    ...
    values = np.asarray(
        [
            ...
            _comparison_value(raw_rows, "F-", "epcsaft", "disp"),
...
def _write_report(disp_rows: list[dict[str, float]], feos_rows: list[dict[str, float]], fluoride_rows: list[dict[str, float]]) -> None:
    mixed_non_f = [row["clapeyron_mixed_minus_epcsaft_kj_mol"] for row in disp_rows if row["ion"] in {"Cl-", "Br-", "I-"}]
    mixed_f = next(row["clapeyron_mixed_minus_epcsaft_kj_mol"] for row in disp_rows if row["ion"] == "F-")
    advanced_f = next(row["clapeyron_advanced_only_disp_kj_mol"] for row in disp_rows if row["ion"] == "F-")
    override_f = next(row["clapeyron_repo_override_minus_epcsaft_kj_mol"] for row in disp_rows if row["ion"] == "F-")
    ...
    feos_disp_residuals = [row["feos_minus_epcsaft_composition_derivative_residual_helmholtz_disp_kj_mol"] for row in feos_rows]
    feos_hc_offsets = [row["feos_minus_epcsaft_composition_derivative_residual_helmholtz_hc_kj_mol"] for row in feos_rows]

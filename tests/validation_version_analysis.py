"""Per-salt NaCl/KBr version analysis across 2008, 2014, and 2020 setups."""
...
def _load_osmotic_data(salt, m_min=0.0, m_max=4.0):
    data_path = Path(__file__).resolve().parents[1] / "data" / "osmotic" / f"{salt}.csv"
...
def _build_params_2020(salt, t):
    """2020 ePC-SAFT advanced settings from Bülow/Ascani/Held 2020 Tables 1-4."""
    sigma_w = _water_sigma(t)

    if salt == "NaCl":
        m = np.asarray([1.0, 1.0, 1.2047], dtype=float)
        s = np.asarray([2.8232, 2.7560, sigma_w], dtype=float)
        e = np.asarray([230.00, 170.00, 353.95], dtype=float)
        k_ij = np.zeros((3, 3), dtype=float)
        k_ij[0, 2] = 0.0045
        k_ij[2, 0] = 0.0045
        k_ij[1, 2] = -0.2500
        ...
        m = np.asarray([1.0, 1.0, 1.2047], dtype=float)
        s = np.asarray([3.3417, 3.0707, sigma_w], dtype=float)
        e = np.asarray([200.00, 190.00, 353.95], dtype=float)
        k_ij = np.zeros((3, 3), dtype=float)
        k_ij[0, 2] = 0.1997
        k_ij[2, 0] = 0.1997
        k_ij[1, 2] = -0.2500
    ...
    params = {
        ...
        "e": e,
        "e_assoc": np.asarray([0.0, 0.0, 2425.7], dtype=float),
        "vol_a": np.asarray([0.0, 0.0, 0.04509], dtype=float),
        "z": np.asarray([1.0, -1.0, 0.0], dtype=float),
        "k_ij": k_ij,
        "dielc": np.asarray([8.0, 8.0, DIELC_CONST], dtype=float),
        "born_model": 1,
        "dielc_rule": 1,
        "DH_model": 1,
        "debug": False,
...
def run_validation_version_analysis():
    ...
    version_colors = {"2008": "gray", "2014": "black", "2020": "tab:blue"}
    version_styles = {"2008": "--", "2014": "-", "2020": "-."}
    output_paths = {
        "NaCl": out_dir / "validation_versions_nacl.png",
        "KBr": out_dir / "validation_versions_kbr.png",
    }

    for salt in ("NaCl", "KBr"):
        m_exp, phi_exp = _load_osmotic_data(salt)
        curves = {
            "2008": _calc_osmotic_curve(salt, m_plot, "2008"),
            "2014": _calc_osmotic_curve(salt, m_plot, "2014"),
            "2020": _calc_osmotic_curve(salt, m_plot, "2020"),
        }
        ...
        for version in ("2008", "2014", "2020"):
            ax.plot(
                ...
                linewidth=1.8 if version != "2014" else 2.0,
                linestyle=version_styles[version],
                label=f"{salt} {version}",
            )
        ...
        ax.set_title(f"{salt}/water molal osmotic coefficients at 298.15 K (2008/2014/2020)")
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=8)
        fig.tight_layout()
        fig.savefig(output_paths[salt], dpi=220)
        plt.close(fig)

        if not output_paths[salt].exists():
            raise FileNotFoundError(f"Expected plot was not written: {output_paths[salt]}")


def test_validation_version_analysis():

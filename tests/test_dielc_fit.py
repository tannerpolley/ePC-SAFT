...
def _load_dielc_data():
    ...
    data_path = Path(__file__).resolve().parents[1] / "data" / "dielc" / "dielc_salts_in_water.csv"
...
def test_dielc_fit():
    ...
    plot_path = out_dir / "test_dielc_fit_salts_in_water.png"
    ...
    ax.plot(x_ion_grid, dielc_rule1, color="black", linestyle="-", linewidth=1.8, label="rule 1 (solid)")
    ax.plot(x_ion_grid, dielc_rule4, color="black", linestyle="--", linewidth=1.8, label="rule 4 (dashed)")

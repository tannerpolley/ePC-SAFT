    ...
    for path in PARAMETER_ROOT.rglob("*.csv"):
        with path.open(newline="", encoding="utf-8") as handle:
            ...
            for line_number, row in enumerate(reader, start=2):
                ...
                assoc_enabled = _assoc_parameter_is_enabled(row.get("e_assoc")) or _assoc_parameter_is_enabled(row.get("vol_a"))
                if assoc_enabled:
                    if token != "2B":
                        offenders.append(f"{path.relative_to(REPO_ROOT)}:{line_number}:{component}: assoc_scheme={token!r}")
                elif token:
                    offenders.append(f"{path.relative_to(REPO_ROOT)}:{line_number}:{component}: non-associating token={token!r}")

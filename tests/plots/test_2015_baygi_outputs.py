...
PLOT_ROOT = REPO_ROOT / "scripts" / "paper_validation" / "2015_Baygi_analysis"
...
def _seed_cached_baygi_outputs() -> None:
    common = importlib.import_module("scripts.paper_validation.2015_Baygi_analysis._common")
    ...
    for figure in EXPECTED_FIGURES:
        common.write_csv_rows(
            PLOT_ROOT / figure / "out" / "data" / f"{figure}_diagnostics.csv", fields, [*dippr, *diagnostics]
        )

    common.write_csv_rows(
        PLOT_ROOT / "out" / "data" / "regressed_parameters.csv",
        (
            "assoc_scheme",
            "m",
            "s",
...
def test_2015_baygi_figure_2_and_3_workflow_outputs_expected_series():
    assert (PLOT_ROOT / "out" / "data" / "regressed_parameters.csv").exists()
    for figure in EXPECTED_FIGURES:
        figure_dir = PLOT_ROOT / figure / "out"
...
def test_2015_baygi_figures_report_numeric_fit_quality():
    figure_2_rows = _diagnostic_rows(PLOT_ROOT / "figure_2" / "out" / "data" / "figure_2_diagnostics.csv")
    figure_3_rows = _diagnostic_rows(PLOT_ROOT / "figure_3" / "out" / "data" / "figure_3_diagnostics.csv")
    figure_2_metrics = _metric_rows(PLOT_ROOT / "figure_2" / "out" / "data" / "figure_2_metrics.csv")
    figure_3_metrics = _metric_rows(PLOT_ROOT / "figure_3" / "out" / "data" / "figure_3_metrics.csv")

...
def test_mea_co2_h2o_benchmark_smoke_fits_only_pure_parameters(tmp_path):
    ...
    assert all(result.backend == "diagnostic_residual_score" for result in results.values())
    assert all(result.optimizer_backend == "diagnostic_residual_score" for result in results.values())
    assert all(result.derivative_backend == "diagnostic_residual_score" for result in results.values())

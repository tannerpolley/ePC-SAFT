...
def test_mea_co2_h2o_benchmark_smoke_fits_only_pure_parameters(tmp_path):
    ...
    assert all(result.optimizer_backend == "not_applicable" for result in results.values())
    assert all(result.derivative_backend == "not_applicable" for result in results.values())

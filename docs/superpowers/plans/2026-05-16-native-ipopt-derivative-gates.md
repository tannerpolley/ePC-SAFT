...
Task 6 continuation note: public `solve_reactive_speciation`, `mixture.chemical_equilibrium`, and `mixture.equilibrium(kind="chemical_equilibrium")` no longer execute the transitional native chemical-equilibrium residual solve when `solver_backend="auto"`. The public auto route validates request shape and reaction-convention support, then selects the native Ipopt homogeneous speciation NLP for ideal, activity, and concentration standard states. Tests that only protected custom residual-solver success were deleted or rewritten as native-Ipopt route/dependency checks, while the private residual evaluator remains covered as derivative-surface diagnostics.

Task 6 continuation note: `solver_backend="auto"` now routes implemented homogeneous `ideal_mole_fraction` reactive speciation to native Ipopt, preserving the requested backend in diagnostics as `auto` and recording `auto_selected_native_ipopt` as the selection reason. Explicit `jacobian_backend="cppad"` requests for ideal homogeneous speciation now use a registered CppAD residual Jacobian, CppAD ideal-Gibbs objective gradient, and CppAD implicit-sensitivity matrix. Activity and concentration residual diagnostics now use the exact CppAD-implicit phase-state derivative surface.

Task 6 continuation note: the `apparent` reaction-constant convention no longer shares the native ideal-speciation standard-state code. It remains accepted as metadata, but any native speciation route request fails before Ipopt until apparent-constant semantics are implemented deliberately. The private chemical-equilibrium residual evaluator stopped claiming analytical Jacobians for activity/concentration residuals; those residual-Jacobian requests now report `cppad_implicit` and use phase-state fugacity/density composition sensitivities.

Task 6 continuation note: Python no longer pre-gates `solver_backend="auto"` reactive-speciation requests solely because a reaction uses activity or concentration standard states. Those requests now build the native chemical-equilibrium payload and route to the native nonideal Gibbs/activity NLP when Ipopt is compiled; no-Ipopt builds fail at the real native Ipopt dependency gate. The obsolete no-reaction mixed-objective status test was deleted, and retained reactive-regression tests now exercise native route ownership or the real Ipopt dependency gate.

Task 6 continuation note: the private native chemical-equilibrium residual evaluator now covers ideal, activity, and
concentration standard-state residuals with exact derivative metadata. Ideal requests keep analytical or CppAD log-amount
Jacobians; activity and concentration requests use the CppAD-implicit phase-state fugacity/density composition sensitivity
surface already used by the native reactive-phase residual diagnostics.
...
Task 6 continuation note: homogeneous nonideal reactive speciation now has a native production route for shared
`mole_fraction_activity` and `concentration` standard-state bases. The NLP uses species amounts, material/charge
constraints, EOS activity or concentration Gibbs terms, exact objective gradients assembled from CppAD-implicit
phase-state fugacity/density composition sensitivities, exact linear constraint Jacobians, one deterministic initial
point, and the generic native Ipopt adapter. No-Ipopt builds now fail at the true native Ipopt dependency gate instead
of a synthetic unavailable-route status. Mixed standard-state bases still fail loudly because a single thermodynamic
objective has not been specified for that contract.
...
Task 6/12 continuation note: the ideal reactive-speciation Ipopt route now reports the positive analytical derivative
backend without no-op activity derivative policy or Hessian-mode diagnostics.

Task 9/12 continuation note: native Ipopt smoke and route result payloads no longer expose Hessian-strategy or
exact-Hessian-required fields; those choices stay inside the adapter while result payloads report exact gradient/Jacobian
requirements.
...
Task 2/12 continuation note: the CppAD dependency row no longer reports a negative production status or EOS-coverage

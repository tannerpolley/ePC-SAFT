...
- Plot asset changes: run the owning ``analyses/<short_id>/scripts`` coordinator or the figure-local ``analyses/<short_id>/figures/<figure_id>/scripts`` entrypoint, plus any targeted opt-in test under ``analyses/package_plot_smokes/tests``, only when regenerating local plot outputs is explicitly part of the task.

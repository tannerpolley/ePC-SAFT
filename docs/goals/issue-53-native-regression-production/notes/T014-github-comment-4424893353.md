...
- backend-unavailable production gate at that boundary
...
2. Backend unavailable debug-only everywhere, gated by `EPCSAFT_ALLOW_DERIVATIVE_BACKEND_DEBUG=1`.
...
7. Tests must prove real thermodynamic parameter movement, objective decrease, non-backend-unavailable derivatives, and no Python production objective.
8. Benchmarks must report `optimizer_backend=ceres`, `native_hot_loop=true`, `python_objective_used=false`, and `derivative_backend != Backend_unavailable`.

...
`verified`: the replay runtime itself is green for the 26 SI rows, but the old package-local calibrated organic payload came from a removed fitting workflow. Future calibration must route through the native regression gate.

...
```powershell
git fetch origin --prune
gh issue view 94 --repo tannerpolley/ePC-SAFT --json number,title,state,body,url
...
1. Inspect GitHub issue #94 and implement only that assigned roadmap task on the assigned branch.
...
5. Open a focused draft PR with the GitHub CLI if no PR exists for the branch, and include Closes #94 in the PR body.
...
10. After merge, confirm issue #94 is closed; if it is still open, close it with a comment that names the merged PR and merge commit.
...
gh issue view 94 --repo tannerpolley/ePC-SAFT --json state,url
gh issue close 94 --repo tannerpolley/ePC-SAFT --comment "Completed by merged PR <number> at <merge commit>."  # only if still open
git fetch origin main --prune
git switch --detach origin/main
git branch -d codex/generic-regression-backend
```
...
git switch --detach origin/main
git branch -d codex/generic-regression-backend
git push origin --delete codex/generic-regression-backend  # only if gh pr merge --delete-branch did not already delete it

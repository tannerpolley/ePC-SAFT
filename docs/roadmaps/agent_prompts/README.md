...
Tasks F-M also include the full completion lifecycle: inspect the corresponding GitHub issue, open a focused draft PR with `Closes #<corresponding issue number>`, self-review against `origin/main`, mark ready after validation, wait for checks, merge without another yes only when checks pass and the final GoalBuddy audit says `full_outcome_complete: true`, confirm or close the issue, then delete both the remote and local task branches. If GitHub, mergeability, or branch cleanup blocks that lifecycle, record the explicit `BLOCKED_*` status in `state.yaml` instead of silently narrowing completion.

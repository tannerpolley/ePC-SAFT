    ...
    for term in terms:
        ...
        for record in term.records:
            ...
            if term.term_type == TERM_DENSITY:
                ...
                basis = str(_value_from_record(record, "activity_basis", "miac_basis", required=False) or "molality").strip().lower()
...
def _fit_binary_pair_internal(
    ...
    *,
    ...
    dataset_obj = _load_dataset(dataset)
    current = {
        target: _matrix_value(dataset_obj, "k_hb" if target == "k_hb_ij" else target, normalized_pair[0], normalized_pair[1], T_ref)
...
def _mea_co2_h2o_species_from_records(records: Sequence[Mapping[str, Any]], species: Iterable[str] | None) -> tuple[str, ...]:
    normalized = _normalize_species_list(species)
...
def _build_mea_co2_h2o_terms(records: Sequence[dict[str, Any]]) -> tuple[FitTerm, ...]:
    ...
    co2_records = [record for record in records if _value_from_record(record, "lnphi_CO2", "ln_fugacity_coefficient_CO2", required=False) is not None]
    osmotic_records = [
        record
        for record in records
        if _value_from_record(record, "osmotic_coefficient", "osmotic", required=False) is not None
...
def _target_bounds(targets: Sequence[str], bounds: FitBounds | Mapping[str, tuple[float | None, float | None]] | None) -> tuple[np.ndarray, np.ndarray]:

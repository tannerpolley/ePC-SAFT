...
void dielectric_inputs_valid_cpp(const vector<double> &x, const add_args &cppargs) {
    ...
    if (cppargs.hc_dadx_diff_mode < 0 || cppargs.hc_dadx_diff_mode > 3 || cppargs.hc_dadx_diff_mode == 1) {
        throw ValueError("Unknown hc_model dadx_differential_mode. Supported values are analytical/autodiff/auto (0/2/3).");
    }
    if (cppargs.disp_dadx_diff_mode < 0 || cppargs.disp_dadx_diff_mode > 3 || cppargs.disp_dadx_diff_mode == 1) {
        throw ValueError("Unknown disp_model dadx_differential_mode. Supported values are analytical/autodiff/auto (0/2/3).");
    }
    if (cppargs.assoc_dadx_diff_mode < 0 || cppargs.assoc_dadx_diff_mode > 3 || cppargs.assoc_dadx_diff_mode == 1) {
        throw ValueError("Unknown assoc_model dadx_differential_mode. Supported values are analytical/autodiff/auto (0/2/3).");
    }
    if (cppargs.born_diff_mode < 0 || cppargs.born_diff_mode > 5 || cppargs.born_diff_mode == 1) {
        throw ValueError(
            "Unknown born_diff_mode. Supported values are 0 (analytic), 2 (Eq.133-style), 3 (no dielectric-concentration term), 4 (autodiff), and 5 (auto)."
        );
    }
    ...
    if (cppargs.mu_DH_diff_mode < 0 || cppargs.mu_DH_diff_mode > 3 || cppargs.mu_DH_diff_mode == 1) {
        throw ValueError("Unknown mu_DH differential_mode. Supported values are analytical/autodiff/auto (0/2/3).");
    }
    ...
    if (cppargs.mu_born_diff_mode < 0 || cppargs.mu_born_diff_mode > 3 || cppargs.mu_born_diff_mode == 1) {
        throw ValueError("Unknown mu_born differential_mode. Supported values are analytical/autodiff/auto (0/2/3).");
    }
...
vector<double> dielectric_derivative_rule_cpp(int rule, const vector<double> &x, const add_args &cppargs) {
    ...
    if (rule == 8) {
        return dielectric_derivative_rule_ad_cpp(rule, x, cppargs);
    }
    if (rule == 2) {
        double mw_bar = 0.0;
...
DielectricState dielectric_state_cpp(const vector<double> &x, const add_args &cppargs) {
    ...
    if (cppargs.dielc_diff_mode == 0) {
        if (cppargs.dielc_rule == 8) {
            throw ValueError("analytical differential_mode is not implemented for dielc_rule=8.");
        }
        ...
        throw ValueError("Unsupported dielc_diff_mode for dielectric derivatives.");
    }
    return state;
}

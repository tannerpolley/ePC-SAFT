#include "epcsaft_contrib_internal.h"

using namespace thermo_detail;

namespace {

template <typename Scalar>
Scalar hs_contact_value_autodiff_cpp(double pair_diameter, const Scalar &zeta2, const Scalar &zeta3) {
    return 1.0 / (1.0 - zeta3)
        + pair_diameter * 3.0 * zeta2 / scalar_pow(1.0 - zeta3, 2)
        + scalar_pow(pair_diameter, 2.0) * 2.0 * zeta2 * zeta2 / scalar_pow(1.0 - zeta3, 3);
}

}  // namespace

double pair_diameter_cpp(double d_i, double d_j) {
    return d_i * d_j / (d_i + d_j);
}

double hs_contact_value_cpp(double pair_diameter, double zeta2, double zeta3) {
    return 1.0 / (1.0 - zeta3)
        + pair_diameter * 3.0 * zeta2 / std::pow(1.0 - zeta3, 2.0)
        + std::pow(pair_diameter, 2.0) * 2.0 * zeta2 * zeta2 / std::pow(1.0 - zeta3, 3.0);
}

HardChainState hard_chain_state_cpp(const MixtureState &thermo, const vector<double> &x, const add_args &cppargs) {
    HardChainState state;
    int ncomp = static_cast<int>(x.size());
    state.zeta.assign(4, 0.0);
    state.ghs.assign(ncomp * ncomp, 0.0);

    for (int k = 0; k < 4; ++k) {
        double summ = 0.0;
        for (int j = 0; j < ncomp; ++j) {
            summ += x[j] * cppargs.m[j] * std::pow(thermo.d[j], k);
        }
        state.zeta[k] = PI / 6.0 * thermo.den * summ;
    }

    state.eta = state.zeta[3];

    int idx = -1;
    for (int i = 0; i < ncomp; ++i) {
        for (int j = 0; j < ncomp; ++j) {
            ++idx;
            double pair_diameter = pair_diameter_cpp(thermo.d[i], thermo.d[j]);
            state.ghs[idx] = hs_contact_value_cpp(pair_diameter, state.zeta[2], state.zeta[3]);
        }
    }

    return state;
}

AutodiffHardChainState hard_chain_state_autodiff_cpp(double den, const vector<double> &d, const vector<AutoDual> &x, const add_args &cppargs) {
    AutodiffHardChainState state;
    int ncomp = static_cast<int>(x.size());
    state.zeta.assign(4, AutoDual(0.0, 0.0));
    state.ghs.assign(ncomp * ncomp, AutoDual(0.0, 0.0));

    for (int k = 0; k < 4; ++k) {
        AutoDual summ = 0.0;
        for (int j = 0; j < ncomp; ++j) {
            summ += x[j] * cppargs.m[j] * scalar_pow(d[j], k);
        }
        state.zeta[k] = PI / 6.0 * den * summ;
    }

    state.eta = state.zeta[3];

    int idx = -1;
    for (int i = 0; i < ncomp; ++i) {
        for (int j = 0; j < ncomp; ++j) {
            ++idx;
            double pair_diameter = pair_diameter_cpp(d[i], d[j]);
            state.ghs[idx] = hs_contact_value_autodiff_cpp(pair_diameter, state.zeta[2], state.zeta[3]);
        }
    }

    return state;
}
